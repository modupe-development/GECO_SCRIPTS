...
my $syntax = "select group_concat(library_id), chrom, position, group_concat(ref_allele), group_concat(alt_allele),
		group_concat(quality), count(library_id) from variants_result where library_id between 1321 and 1337 and
