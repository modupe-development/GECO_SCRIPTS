...
$sth = $dbh->prepare("insert into bird_libraries (library_id,bird_id, species, line, tissue, method, index_, chip_result, scientist,date, notes) 
 ...
 while (<BIRDS>){
 	...
 	my @allofit = split("\t", $_);
 	if ($allofit[0] =~ /^\d/){ 
 	   unless (exists $Hashresults{$allofit[0]}){
           # my $lib_id6 = $allofit[0]."DE"; #library_id
         ...
         #                     tissue=\"$allofit[4]\",method=\"$allofit[7]\", index_=\"$allofit[8]\", chip_result=\"$allofit[10]\",
          #                    scientist=\"$allofit[13]\",date=\"$allofit[14]\",notes=\"$allofit[15]\" where library_id=$allofit[0]");
            $sth->execute($allofit[0],$allofit[1],$allofit[2],$allofit[3],$allofit[4],$allofit[7],$allofit[8],$allofit[10],$allofit[13],
                      $allofit[14],$allofit[15]);
         }
 	}
 } 
 close BIRDS;
...
#$sth = $dbh->prepare($syntax);
#$sth->execute or die "SQL Error: $DBI::errstr\n";
#while (@row = $sth->fetchrow_array() ) {print "Number of rows in \"variants_list\" table \t:\t @row\n";}
#
#$syntax = "select count(*) from bird_libraries";
#$sth = $dbh->prepare($syntax);
#$sth->execute or die "SQL Error: $DBI::errstr\n";
#while (@row = $sth->fetchrow_array() ) {print "Number of rows in \"bird_libraries\" table \t:\t @row\n";}
